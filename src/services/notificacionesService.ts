...
class NotificacionesService {
     ...
     */
    async obtenerNotificaciones(usuarioId: number, mostrarLeidas: boolean): Promise<Notificacion[]> {
     ...
     */
    async marcarComoLeidas(usuarioId: number, ids: number[]): Promise<void> {
        // Solo permite que el usuario actual marque como leídas sus propias notificaciones
        await notificacionRepo.update(
            { id: In(ids), usuarioId }, 
            { leida: true }
        );
    }
